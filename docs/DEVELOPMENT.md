...
Event titles are prefixed with the BrewPack or schedule name (for example, `Dark Matter: Tap day` or `Dark Matter - Custom: Start fermentation`), and each description includes the schedule name, style and ABV when available, the stage, the selected timing mode for official BrewPacks, the total lead time, a note that the schedule was generated by Tap Planner, and the live application URL.
...
Because all-day iCalendar end dates are **exclusive**, `DTEND` is always the calendar day **after** `DTSTART`, so each stage occupies exactly one day.

### Privacy

Export happens entirely in the browser via a `Blob` and an object URL (revoked after download). Event UIDs are derived from the stage name, schedule name, and stage date, so no database is needed. **No calendar account access is requested**, no external calendar API is called, and no calendar data is stored. After a successful download the result card announces a confirmation through `aria-live="polite"`; it never claims that events were added to a calendar automatically.
